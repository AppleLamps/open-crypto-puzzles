#!/usr/bin/env python3
"""
fig_blocks.py -- generate images/02-structure-blocks.svg.

Purpose:
    Draw the Quizchain series structure as a left-to-right row of solved
    stages (blue) that forks into the two still-open blocks (orange), each
    pointing at its own escrow address and balance. This is the structural
    fact behind the puzzle's current state: of roughly 90 published blocks
    across both rounds, every one but these two was claimed by readers in
    2019, confirmed by an exhaustive sweep of all 159 funding transactions
    cited in the author's own posts.

    Each gate's escrow address is shortened to first6...last6 on the figure
    (the full address stays in the README prose and in data/blocks-structure.json);
    the canvas width is then computed from the actual widest line so nothing runs
    past the right edge.

Usage:
    python3 tools/fig_blocks.py

Input:
    data/blocks-structure.json (chain stages and the two open gates).

Output:
    images/02-structure-blocks.svg
"""
...
DATA_PATH = os.path.join(ROOT, "data", "blocks-structure.json")
OUT_PATH = os.path.join(ROOT, "images", "02-structure-blocks.svg")
...
def main():
    ...
    parts.append(f'<text x="{MARGIN}" y="22" font-size="13" font-weight="bold" '
                 f'fill="{COLOR_TITLE}">Quizchain series, both rounds (claimed 2019-2019)</text>')

    y = height // 2 - BOX_H // 2 + 6
    x = MARGIN
    centers = []
    for stage in chain:
    ...
    parts.append(f'<text x="{MARGIN + 20}" y="{legend_y + 11}" font-size="10" '
                 f'fill="{COLOR_TITLE}">solved and claimed (2019)</text>')
    ...
    parts.append(f'<text x="{MARGIN + 250}" y="{legend_y + 11}" font-size="10" '
                 f'fill="{COLOR_TITLE}">open, funded and unspent</text>')
    max_right = max(max_right, MARGIN + 250 + len("open, funded and unspent") * 10 * 0.62)
