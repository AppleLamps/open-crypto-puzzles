#!/usr/bin/env python3
"""
fig_pipeline.py -- generate images/02-pipeline-derivation.svg.

Purpose:
    Draw the final-gate derivation pipeline (candidate answer to P2PKH address) as a
    left-to-right chain of boxes. This is the transform tools/oracle.py implements,
    certified in two independent parts (see README, "Certified against"): the
    address-derivation half against the escrow's own on-chain public key, and the
    AES-256-CBC decrypt half against a self-made round-trip vector.
...
def main():
    ...
    for i in range(1, n):
        ...
        if edge_label:
            ...
            line_h = 12
            base_y = mid_y - 20 - (len(edge_lines) - 1) * line_h
