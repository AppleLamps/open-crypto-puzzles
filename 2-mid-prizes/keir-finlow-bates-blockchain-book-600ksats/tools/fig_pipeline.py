...
Purpose:
    Draw the sha256x3 derivation pipeline (answer string to P2PKH address) as a
    left-to-right chain of boxes. This is the confirmed transform for the two solved
    lots used to certify the oracle (EN_easy_1, IT_hard) and the working hypothesis for
    the 3 lots still open.
...
def main():
    ...
    # arrows and edge labels, drawn after boxes so labels sit above the connecting line
    for i in range(1, n):
