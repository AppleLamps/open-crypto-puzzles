#!/usr/bin/env python3
"""
oracle.py -- candidate checker for the LuckyLurker "Bitcoin Vault" puzzle.

A candidate is a 12-word BIP39 English mnemonic (space-separated), one word per
published hint, in order (hint N = seed word N; the puzzle page's own labeling
fixes the position, so word order is not a separate unknown). It is validated as
BIP39 checksum-correct, derived to a Bitcoin address under BIP84
(m/84'/0'/0'/0/0, no passphrase), and compared to the escrow address. BIP49 and
BIP44 first addresses are also checked as a fallback, since the puzzle page never
states the derivation path directly.
